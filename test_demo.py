...
print("待测试向量差装载完成")
succes_num = 0
for data_set_index in tqdm(range(len(logicfile_set)), desc="处理数据集"):
	data_set = logicfile_set[data_set_index]
	target_id = data_set["word_id"]
	answer_id = data_set["master_id"]
	target = GKSD_operator.safe_db_operation("search", id_num=target_id, log_printing=False)[0]
...
for data_set_index in tqdm(range(len(logicfile_set)), desc="处理数据集"):
	...
	delta_np = np.array(delta_v)
	answer_vector = (vector_np + delta_np).tolist()
	answer_list = GKSD_operator.safe_db_operation("search", vector=answer_vector, log_printing=False, limit=20)
	if answer_id in [answer["id"] for answer in answer_list]:
		succes_num += 1
point = (succes_num / len(logicfile_set)) * 100
print("运算完毕 结果20\n成功率 ", point, "%")
...
for data_set_index in tqdm(range(len(logicfile_set)), desc="处理数据集"):
	...
	delta_np = np.array(delta_v)
	answer_vector = (vector_np + delta_np).tolist()
	answer_list = GKSD_operator.safe_db_operation("search", vector=answer_vector, log_printing=False, limit=40)
	if answer_id in [answer["id"] for answer in answer_list]:
		succes_num += 1
point = (succes_num / len(logicfile_set)) * 100
print("运算完毕 结果40\n成功率 ", point, "%")
