...
'''
脚本任务：
- 遍历标准词汇表，并半自动判断其对应IsA名称。
- 查询IsA名称，若词汇表中缺失则自动添加。
- 若成功查询IsA则计算IsA的向量相对位置，并将差值计算并统计。
- 求出所有有效三元组的向量平均差距。
- 反向验证有效性。
'''
...
def process_main(index_for_now):
	...
	try:
		...
		master_word = ai_modules.logic_IsA(word)
		if master_word == ">无结果<" :
			level = 20
			raise Exception("未找到有效IsA结果")
		...
		IsA_vector = np.array(master_vector) - np.array(vector)
		if isinstance(IsA_vector, np.ndarray):
			IsA_vector = IsA_vector.tolist()
		...
		with open('IsA.json', 'a', encoding='utf-8') as file:
